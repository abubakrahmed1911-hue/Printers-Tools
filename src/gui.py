...
TRANSLATIONS = {
    "ar": {
        ...
        # ── Network scan + Remove printer ──
        "Scan Network for Printers":        "فحص الشبكة عن طابعات",
        "Scan Again":                       "فحص مجدداً",
        "Scanning network for printers...": "جاري فحص الشبكة...",
        "This may take up to 30 seconds":   "قد يستغرق حتى 30 ثانية",
        "Scan failed":                      "فشل الفحص",
        "No network printers found":        "لم يتم العثور على طابعات شبكة",
        "Remove a Printer":                 "حذف طابعة",
        "Loading printer list...":          "جاري تحميل قائمة الطابعات...",
        "Select a printer to remove:":      "اختر الطابعة للحذف:",
        "No printers found":                "لا توجد طابعات",
        "This will delete the printer from CUPS permanently.": "سيتم حذف الطابعة من CUPS بشكل دائم.",
        "Reset Print Spooler?":             "إعادة ضبط الطباعة؟",
        "This will stop CUPS, clear all print queues, and restart. All pending jobs will be lost.":
            "سيوقف CUPS ويمسح طابور الطباعة ويعيد التشغيل. ستُفقد الأوامر المعلقة.",
        "Resetting print spooler...":       "جاري إعادة ضبط الطباعة...",
        "Print spooler reset successfully!":"تمت إعادة ضبط الطباعة بنجاح!",
        "Quick Fix Print Spooler":          "إصلاح سريع لنظام الطباعة",

        # ── Thermal brand flow ──
        "Connect the printer via USB, then choose your brand:":
            "وصّل الطابعة بـ USB ثم اختر البراند:",
        "Install Selected Brand Driver":    "تثبيت دراير البراند المختار",
        "Install Generic Driver":           "تثبيت دراير عام",
        "Advanced: install generic CUPS thermal driver":
            "متقدم: تثبيت دراير CUPS حراري عام",
        "No USB printer detected. Connect and try again.":
            "لم يتم اكتشاف طابعة USB. وصّلها وحاول مجدداً.",
        "Multiple USB printers — pick one:":"أكثر من طابعة USB — اختر واحدة:",
        "Printer":                          "طابعة",
        "Branch":                           "الفرع",

        # ── t_fmt keys with {} ──
        "Detecting USB devices for {}...":  "جاري اكتشاف USB لـ {}...",
        "Installing {}... may take a minute": "جاري تثبيت {}... قد يستغرق دقيقة",
        "Found {} printer(s):":             "تم العثور على {} طابعة:",
        "Your IP: {} — subnet scanned":     "IP الخاص بك: {} — تم فحص الـ subnet",
        "Remove printer: {}?":              "حذف الطابعة: {}؟",
        "Removing {}...":                   "جاري حذف {}...",
        "Failed: {}":                       "فشل: {}",
        "Status":                           "الحالة",
        "Thermal":                          "حرارية",
...
class ITAmanApp(Gtk.Window):
    ...
    def _setup_show_search(self):
        ...
        btn_row.pack_start(
            btn(t("Scan Network for Printers"), "btn-primary",
                lambda _: self._run_network_scan()), False, False, 0)
        btn_row.pack_start(
            btn(t("Remove a Printer"), "btn-danger",
                lambda _: self._show_remove_printer()), False, False, 0)
        self.setup_content.pack_start(btn_row, False, False, 6)
        sep = Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL)
        sep.set_margin_bottom(4)
        self.setup_content.pack_start(sep, False, False, 0)
    ...
    def _run_network_scan(self):
        _clear(self.setup_content)
        sp = Gtk.Spinner(); sp.set_size_request(44, 44); sp.start()
        self.setup_content.pack_start(lbl(t("Scanning network for printers..."), "spinner-label"), False, False, 6)
        self.setup_content.pack_start(sp, False, False, 6)
        self.setup_content.pack_start(lbl(t("This may take up to 30 seconds"), "status-msg"), False, False, 4)
        self.setup_content.pack_start(self._make_back_button(lambda _: self._setup_show_search()), False, False, 5)
        self.setup_content.show_all()
        def task(): return send_command({"action": "network_scan"})
        def cb(r):
            sp.stop(); sp.set_visible(False)
            self._show_network_results(r)
        run_in_thread(task, cb)

    def _show_network_results(self, result):
        _clear(self.setup_content)
        self.setup_content.pack_start(self._make_back_button(lambda _: self._setup_show_search()), False, False, 5)
        if result.get("status") != "ok":
            self.setup_content.pack_start(lbl(result.get("message", t("Scan failed")), "error-msg"), False, False, 8)
            self.setup_content.pack_start(btn(t("Try Again"), "btn-primary", lambda _: self._run_network_scan()), False, False, 5)
            self.setup_content.show_all(); return
        printers = result.get("printers", [])
        local_ip = result.get("local_ip", "")
        if local_ip:
            self.setup_content.pack_start(lbl(t_fmt("Your IP: {} — subnet scanned", local_ip), "status-msg"), False, False, 4)
        if not printers:
            self.setup_content.pack_start(lbl(t("No network printers found"), "status-msg"), False, False, 8)
            self.setup_content.pack_start(btn(t("Scan Again"), "btn-primary", lambda _: self._run_network_scan()), False, False, 8)
            self.setup_content.show_all(); return
        self.setup_content.pack_start(lbl(t_fmt("Found {} printer(s):", len(printers)), "section-header"), False, False, 6)
        for p in printers:
            ip = p.get("ip",""); uri = p.get("uri", f"ipp://{ip}/ipp/print"); model = p.get("model", f"Printer @ {ip}")
            card = Gtk.Button(); card.get_style_context().add_class("printer-card")
            ib = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            ib.pack_start(lbl(model, "printer-name"), False, False, 0)
            ib.pack_start(lbl(f"IP: {ip}", "printer-detail"), False, False, 0)
            card.add(ib)
            def _install(w, _ip=ip, _m=model): self._install_from_scan(_ip, _m)
            card.connect("clicked", _install)
            self.setup_content.pack_start(card, False, False, 3)
        self.setup_content.show_all()

    def _install_from_scan(self, ip: str, model: str):
        _clear(self.setup_content)
        sp = Gtk.Spinner(); sp.set_size_request(40, 40); sp.start()
        self.setup_content.pack_start(lbl(t_fmt("Installing {}...", model), "spinner-label"), False, False, 5)
        self.setup_content.pack_start(sp, False, False, 6)
        self.setup_content.pack_start(self._make_back_button(lambda _: self._run_network_scan()), False, False, 5)
        steps_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        steps_box.set_margin_start(20); steps_box.set_margin_top(8)
        self.setup_content.pack_start(steps_box, False, False, 0)
        self.setup_content.show_all()
        def task(): return send_command({"action": "setup_printer", "name": "printer-FS", "ip": ip, "model": model})
        def cb(result):
            sp.stop(); sp.set_visible(False)
            for s in result.get("steps", []):
                st = "step-success" if s.get("status") in ("success","warning","skipped") else "step-failed"
                ico = "[OK]" if s.get("status") in ("success","warning","skipped") else "[X]"
                row = lbl(f"{ico} {s.get('step','')} — {s.get('message','')}", st)
                row.set_xalign(0.0); steps_box.pack_start(row, False, False, 0)
            steps_box.pack_start(Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL), False, False, 6)
            if result.get("status") == "ok":
                steps_box.pack_start(lbl(t_fmt("{} installed successfully!", model), "step-success"), False, False, 5)
                global _scan_cache, _scan_cache_time; _scan_cache = None; _scan_cache_time = 0.0
            else:
                steps_box.pack_start(lbl(t_fmt("Failed: {}", result.get("message","")), "step-failed"), False, False, 5)
            steps_box.show_all()
        run_in_thread(task, cb)

    # ── Remove Printer ────────────────────────────────────────────────────────
    def _show_remove_printer(self):
        _clear(self.setup_content)
        self.setup_content.pack_start(self._make_back_button(lambda _: self._setup_show_search()), False, False, 5)
        sp = Gtk.Spinner(); sp.set_size_request(36, 36); sp.start()
        self.setup_content.pack_start(lbl(t("Loading printer list..."), "spinner-label"), False, False, 5)
        self.setup_content.pack_start(sp, False, False, 4)
        self.setup_content.show_all()
        def task(): return send_command({"action": "scan"})
        def cb(r):
            sp.stop(); sp.set_visible(False)
            _clear(self.setup_content)
            self.setup_content.pack_start(self._make_back_button(lambda _: self._setup_show_search()), False, False, 5)
            printers = r.get("printers", [])
            if not printers:
                self.setup_content.pack_start(lbl(t("No printers found"), "status-msg"), False, False, 10)
                self.setup_content.show_all(); return
            self.setup_content.pack_start(lbl(t("Select a printer to remove:"), "section-header"), False, False, 6)
            for p in printers:
                name = p.get("name",""); uri = p.get("uri",""); status = p.get("status","")
                card = Gtk.Button(); card.get_style_context().add_class("printer-card")
                ib = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
                ib.pack_start(lbl(name, "printer-name"), False, False, 0)
                ib.pack_start(lbl(f"{uri}  •  {status}", "printer-detail"), False, False, 0)
                card.add(ib)
                def _remove(w, n=name): self._confirm_remove_printer(n)
                card.connect("clicked", _remove)
                self.setup_content.pack_start(card, False, False, 3)
            self.setup_content.show_all()
        run_in_thread(task, cb)

    def _confirm_remove_printer(self, name: str):
        dialog = Gtk.MessageDialog(
            transient_for=self, flags=0,
            message_type=Gtk.MessageType.WARNING,
            buttons=Gtk.ButtonsType.OK_CANCEL,
            text=t_fmt("Remove printer: {}?", name))
        dialog.format_secondary_text(t("This will delete the printer from CUPS permanently."))
        resp = dialog.run(); dialog.destroy()
        if resp != Gtk.ResponseType.OK: return
        _clear(self.setup_content)
        sp = Gtk.Spinner(); sp.set_size_request(40, 40); sp.start()
        self.setup_content.pack_start(lbl(t_fmt("Removing {}...", name), "spinner-label"), False, False, 5)
        self.setup_content.pack_start(sp, False, False, 6)
        self.setup_content.show_all()
        def task(): return send_command({"action": "remove_printer", "name": name})
        def cb(r):
            sp.stop(); sp.set_visible(False)
            _clear(self.setup_content)
            msg = r.get("message", "")
            css = "step-success" if r.get("status") == "ok" else "step-failed"
            self.setup_content.pack_start(lbl(msg, css), False, False, 10)
            self.setup_content.pack_start(self._make_back_button(lambda _: self._setup_show_search()), False, False, 5)
            self.setup_content.show_all()
            if r.get("status") == "ok":
                global _scan_cache, _scan_cache_time; _scan_cache = None; _scan_cache_time = 0.0
    ...
    def _thermal_show_main(self):
        ...
        self._selected_thermal_brand = getattr(self, "_selected_thermal_brand", "xprinter")

        self.thermal_content.pack_start(lbl(t("Connect the printer via USB, then choose your brand:"), "status-msg"), False, False, 6)

        # Brand cards
        brand_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=14)
        brand_row.set_halign(Gtk.Align.CENTER)
        brand_btns = {}
        for bid, bname, bdesc in [("xprinter", "X-Printer", "XP-80 Series"),
                                    ("sprt", "SPRT", "80mm Thermal")]:
            card = Gtk.Button()
            card.get_style_context().add_class("printer-card")
            if bid == self._selected_thermal_brand:
                card.get_style_context().add_class("printer-card-ok")
            ib = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=3)
            ib.set_halign(Gtk.Align.CENTER)
            ib.set_margin_top(10); ib.set_margin_bottom(10)
            ib.set_margin_start(20); ib.set_margin_end(20)
            ib.pack_start(lbl(bname, "printer-name"), False, False, 0)
            ib.pack_start(lbl(bdesc, "printer-detail"), False, False, 0)
            card.add(ib); card.set_size_request(150, 70)
            brand_btns[bid] = card
            def _pick(w, b=bid):
                self._selected_thermal_brand = b
                for k, v in brand_btns.items():
                    if k == b: v.get_style_context().add_class("printer-card-ok")
                    else: v.get_style_context().remove_class("printer-card-ok")
            card.connect("clicked", _pick)
            brand_row.pack_start(card, False, False, 0)
        self.thermal_content.pack_start(brand_row, False, False, 10)

        self.thermal_content.pack_start(
            btn(t("Install Selected Brand Driver"), "btn-success",
                lambda _: self._thermal_detect_and_install()), False, False, 5)

        sep = Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL)
        sep.set_margin_top(8); sep.set_margin_bottom(4)
        self.thermal_content.pack_start(sep, False, False, 0)
        self.thermal_content.pack_start(lbl(t("Advanced: install generic CUPS thermal driver"), "status-msg"), False, False, 3)
        self.thermal_content.pack_start(
            btn(t("Install Generic Driver"), "btn-primary",
                lambda _: self._thermal_install_driver()), False, False, 5)
    ...
    def _thermal_detect_and_install(self):
        ...
        label = "X-Printer XP-80" if brand == "xprinter" else "SPRT 80mm"
        sp = Gtk.Spinner(); sp.set_size_request(40, 40); sp.start()
        self.thermal_content.pack_start(lbl(t_fmt("Detecting USB devices for {}...", label), "spinner-label"), False, False, 5)
        ...
        self.thermal_content.show_all()
        def task(): return send_command({"action": "detect_usb_printers"})
        def cb(r):
            sp.stop(); sp.set_visible(False)
            usb_list = r.get("usb_printers", []) if r.get("status") == "ok" else []
            if not usb_list:
                _clear(self.thermal_content)
                self.thermal_content.pack_start(lbl(t("No USB printer detected. Connect and try again."), "status-msg"), False, False, 8)
                self.thermal_content.pack_start(btn(t("Try Again"), "btn-primary", lambda _: self._thermal_detect_and_install()), False, False, 5)
                self.thermal_content.pack_start(self._make_back_button(lambda _: self._thermal_show_main()), False, False, 5)
                self.thermal_content.show_all(); return
            uri = usb_list[0]["uri"] if len(usb_list) == 1 else None
            if uri: self._thermal_run_brand_install(brand, uri)
            else: self._thermal_pick_usb(usb_list, brand)
        run_in_thread(task, cb)

    def _thermal_pick_usb(self, usb_list, brand):
        _clear(self.thermal_content)
        self.thermal_content.pack_start(lbl(t("Multiple USB printers — pick one:"), "section-header"), False, False, 6)
        self.thermal_content.pack_start(self._make_back_button(lambda _: self._thermal_show_main()), False, False, 5)
        for i, up in enumerate(usb_list):
            uri = up.get("uri",""); desc = up.get("description", up.get("make_and_model","Unknown"))
            c = Gtk.Button(); c.get_style_context().add_class("printer-card")
            ib = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
            ib.pack_start(lbl(f"USB #{i+1} — {desc}", "printer-name"), False, False, 0)
            ib.pack_start(lbl(uri, "printer-detail"), False, False, 0)
            c.add(ib)
            def _pick(w, u=uri): self._thermal_run_brand_install(brand, u)
            c.connect("clicked", _pick)
            self.thermal_content.pack_start(c, False, False, 2)
        self.thermal_content.show_all()

    def _thermal_run_brand_install(self, brand, usb_uri):
        _clear(self.thermal_content)
        label = "X-Printer XP-80" if brand == "xprinter" else "SPRT 80mm"
        sp = Gtk.Spinner(); sp.set_size_request(40, 40); sp.start()
        self.thermal_content.pack_start(lbl(t_fmt("Installing {}... may take a minute", label), "spinner-label"), False, False, 5)
        self.thermal_content.pack_start(sp, False, False, 6)
        self.thermal_content.pack_start(self._make_back_button(lambda _: self._thermal_show_main()), False, False, 5)
        steps_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)
        steps_box.set_margin_start(20); steps_box.set_margin_top(8)
        ...
        self.thermal_content.show_all()
        def task(): return send_command({"action": "install_thermal_brand", "brand": brand, "usb_uri": usb_uri})
        def cb(result):
            sp.stop(); sp.set_visible(False)
            for s in result.get("steps", []):
                st = "step-success" if s.get("status") in ("success","skipped","warning") else "step-failed"
                ico = "[OK]" if s.get("status") in ("success","skipped","warning") else "[X]"
                row = lbl(f"{ico} {s.get('step','')} — {s.get('message','')}", st)
                row.set_xalign(0.0); steps_box.pack_start(row, False, False, 0)
            steps_box.pack_start(Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL), False, False, 6)
            if result.get("status") == "ok":
                steps_box.pack_start(lbl(result.get("message",""), "step-success"), False, False, 5)
                global _scan_cache, _scan_cache_time; _scan_cache = None; _scan_cache_time = 0.0
            else:
                steps_box.pack_start(lbl(t_fmt("Failed: {}", result.get("message","")), "step-failed"), False, False, 5)
            steps_box.show_all()
    ...
    # ── Async Quick Fix ──
    def _quick_fix_spooler(self):
        """Stop CUPS, wipe spool, restart — full spooler reset."""
        dialog = Gtk.MessageDialog(
            transient_for=self, flags=0,
            message_type=Gtk.MessageType.WARNING,
            buttons=Gtk.ButtonsType.OK_CANCEL,
            text=t("Reset Print Spooler?"))
        dialog.format_secondary_text(
            t("This will stop CUPS, clear all print queues, and restart. All pending jobs will be lost."))
        resp = dialog.run(); dialog.destroy()
        if resp != Gtk.ResponseType.OK: return
        ...
        _clear(self.fix_content)
        sp = Gtk.Spinner(); sp.set_size_request(40, 40); sp.start()
        self.fix_content.pack_start(lbl(t("Resetting print spooler..."), "spinner-label"), False, False, 5)
        self.fix_content.pack_start(sp, False, False, 6)
        self.fix_content.show_all()

        def task(): return send_command({"action": "quick_fix_spooler"})
        def cb(result):
            sp.stop(); sp.set_visible(False)
            _clear(self.fix_content)
            if result.get("status") == "ok":
                self.fix_content.pack_start(lbl(t("Print spooler reset successfully!"), "step-success"), False, False, 10)
                global _scan_cache, _scan_cache_time; _scan_cache = None; _scan_cache_time = 0.0
            else:
                self.fix_content.pack_start(lbl(result.get("message","Failed"), "step-failed"), False, False, 10)
            self.fix_content.pack_start(self._make_back_button(lambda _: self._navigate_back()), False, False, 5)
            self.fix_content.show_all()
        run_in_thread(task, cb)
